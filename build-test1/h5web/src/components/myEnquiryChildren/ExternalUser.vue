...
// 
	...
	export default {
		...
		methods: {
			getNumber(){
				this.$http.post('/wx/admin/spotSale/mySpotSalePurchase?status=' + this.statu+ '&buySell=1'+'&pageSize=1000' + this.searchRequest ).then((response) => {
			...
			getsearch(){
				...
				this.$http.post('/wx/admin/spotSale/mySpotSalePurchase?status=' + status + '&search='+this.searchTxt+'&pageSize=1000').then((response) => {
					this.dataVal = [];
					this.dataList = [];
			...
			editLis1() {
				...
				this.$HttpRequest({
					url: '/wx/admin/spotSale/submit_hanging_dish',
					headers: {
			...
			editLis2() {
				...
				this.RequestFlag = true
				this.$HttpRequest({
					url: '/wx/admin/spotSale/modify_hanging_dish',
					headers: {
						'Content-Type': 'application/json;charset=utf-8',
					},
					method: 'post',
			...
			editLis3() {
				...
				this.RequestFlag = true
				this.$HttpRequest({
					url: '/wx/admin/spotSale/submit_bidding',
					headers: {
						'Content-Type': 'application/json;charset=utf-8',
					},
					method: 'post',
			...
			editLis4() {
				...
				this.RequestFlag = true
				this.$HttpRequest({
					url: '/wx/admin/spotSale/submit_revoke',
					headers: {
						'Content-Type': 'application/json;charset=utf-8',
					},
					method: 'post',
			...
			switchOrder(ind) {
				...
				if (this.searchTxt != '')
				this.searchRequest = `&search=${this.searchTxt}`
				this.$http.post('/wx/admin/spotSale/mySpotSalePurchase?status=' + status + '&buySell=1'+'&pageSize=1000' + this.searchRequest ).then((response) => {
					this.statu = ind;
					console.log(ind)
					this.dataVal = [];
					this.dataList = [];
