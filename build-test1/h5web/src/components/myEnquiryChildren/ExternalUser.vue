...
// 
	...
	export default {
		...
		methods: {
			getNumber(){
				this.$http.post('/wx/admin/spotSale/mySpotSalePurchase?status=' + this.statu+ '&buySell=1'+'&pageSize=1000').then((response) => {
			...
			switchOrder(ind) {
				...
				this.forFlag = false;
				this.loading = true;
				this.$http.post('/wx/admin/spotSale/mySpotSalePurchase?status=' + status + '&buySell=1'+'&pageSize=1000').then((response) => {
					this.statu = ind;
					console.log(ind)
					this.dataVal = [];
					this.dataList = [];
