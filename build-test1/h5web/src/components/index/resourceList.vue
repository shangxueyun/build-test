<template>
	<div class="ziyuan_wrap">
		<div class="ziyuan" v-if="ListShow == 1">
		...
		<div class="res_list this_res" v-if="ListShow == 1">
			<ul>
		...
		<div class="ziyuan" v-if="ListShow == 2">
		...
		<div class="res_list this_res" v-if="ListShow == 2">
			<ul>
		...
		<div class="res_list this_res" style="padding-bottom: 1rem;" v-if="ListShow == 3">
			<ul>
				<li class="res_inx" v-for="item in resList3" @click="godetails(item.batchID,item.listingID,item.listingType)" :key="item.batchID">
					<div class="res_c" style="width: 100%;">
						<div class="res_top wrap">
							<!-- <p v-if="item.listingType==1" class="top_cla">自营</p> -->
							<p class="top_num">{{item.batchID}}</p>
...
<script>
	export default {
		...
		props:{
			paramsList: [String, Object, Array]
		},
		created() {
			...
			this.getList()
				...
				this.ListShow = 3;
				this.resList3 = JSON.parse(JSON.stringify(this.paramsList))
			}
		},
		mounted() {
		...
		methods: {
			paramsUpdate (obj, flg) {
				this.ListShow = 0
				if (flg)
				this.getList()
				else {
					this.ListShow = 3;
					this.resList3 = JSON.parse(JSON.stringify(obj))
					this.$set(this.resList3)
				}
			},
			getList () {
